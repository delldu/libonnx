...
static window_context_t *window_context_alloc(void) {
  window_context_t *wctx;
  Uint32 r, g, b, a;
  int bpp;

  wctx = malloc(sizeof(window_context_t));
  if (!wctx)
    return NULL;
  ...
  wctx->window = SDL_CreateWindow(
      "The mnist handwritten digit prediction "
  ...
  wctx->surface = SDL_CreateRGBSurface(SDL_SWSURFACE, wctx->width, wctx->height,
                                       bpp, r, g, b, a);
...
static void window_draw_progress(window_context_t *wctx, int n, int percent) {
  ...
  r.w = w - 50;
  r.h = h;
  SDL_FillRect(wctx->surface, &r,
               SDL_MapRGB(wctx->surface->format, 240, 240, 240));
  ...
  r.h = h;
  SDL_FillRect(wctx->surface, &r,
               SDL_MapRGB(wctx->surface->format, 200, 200, 200));
}

static void onnx_tensor_apply_image(onnx_tensor_t *y, SDL_Surface *x) {
  float *py = y->datas;
...
static void onnx_tensor_softmax(onnx_tensor_t *x, float *results) {
...
int main(int argc, char *argv[]) {
  ...
  while (!done) {
    ...
    SDL_SetClipRect(wctx->surface,
                    &(SDL_Rect){0, 0, SDL_GetWindowSurface(wctx->window)->w,
                                SDL_GetWindowSurface(wctx->window)->h});
    SDL_FillRect(wctx->surface,
                 &(SDL_Rect){0, 0, SDL_GetWindowSurface(wctx->window)->w,
                             SDL_GetWindowSurface(wctx->window)->h},
                 0xffffffff);
