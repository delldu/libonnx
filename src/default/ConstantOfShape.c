  ...
  if ((n->ninput == 1) && (n->noutput == 1)) {
    ...
    if (pdat) {
      for (i = 0; i < n->proto->n_attribute; i++) {
        ...
        if ((attr->type == ONNX__ATTRIBUTE_PROTO__ATTRIBUTE_TYPE__TENSOR) &&
            (strcmp(attr->name, "value") == 0)) {
          t = attr->t;
          break;
        }
      }
...
static int ConstantOfShape_reshape(onnx_node_t *n) { return 1; }

static void ConstantOfShape_operator(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
