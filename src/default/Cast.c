...
static int Cast_init(onnx_node_t *n) {
  ...
  if ((n->ninput == 1) && (n->noutput == 1)) {
    ...
    if (pdat) {
      pdat->to = (enum onnx_tensor_type_t)onnx_attribute_read_int(
          n, "to", n->inputs[0]->type);
      n->priv = pdat;
      return 1;
    }
  }
...
static void Cast_bool(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%u", (px[i] != 0) ? 1 : 0);
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_int8(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%d", px[i]);
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_int64(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%ld", px[i]);
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_uint8(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%u", px[i]);
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_uint64(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%lu", px[i]);
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_bfloat16(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%g", bfloat16_to_float32(px[i]));
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_float16(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%g", float16_to_float32(px[i]));
      py[i] = strdup(buf);
    }
  } break;
  default:
...
static void Cast_float32(onnx_node_t *n) {
  ...
  case ONNX_TENSOR_TYPE_STRING: {
    ...
    for (i = 0, l = y->ndata; i < l; i++) {
      if (py[i])
        free(py[i]);
      snprintf(buf, sizeof(buf), "%g", px[i]);
      py[i] = strdup(buf);
    }
  } break;
  default:
