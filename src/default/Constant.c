    ...
    if (attr) {
      ...
      case ONNX__ATTRIBUTE_PROTO__ATTRIBUTE_TYPE__FLOATS:
        if ((strcmp(attr->name, "value_floats") == 0) && (attr->n_floats > 0)) {
          if ((y->ndim != 1) || (y->dims[0] != attr->n_floats) ||
              (y->type != ONNX_TENSOR_TYPE_FLOAT32))
            onnx_tensor_reinit(y, ONNX_TENSOR_TYPE_FLOAT32,
                               (int[]){attr->n_floats}, 1);
      ...
      case ONNX__ATTRIBUTE_PROTO__ATTRIBUTE_TYPE__INTS:
        if ((strcmp(attr->name, "value_ints") == 0) && (attr->n_ints > 0)) {
          if ((y->ndim != 1) || (y->dims[0] != attr->n_ints) ||
              (y->type != ONNX_TENSOR_TYPE_INT64))
            onnx_tensor_reinit(y, ONNX_TENSOR_TYPE_INT64, (int[]){attr->n_ints},
                               1);
      ...
      case ONNX__ATTRIBUTE_PROTO__ATTRIBUTE_TYPE__STRINGS:
        if ((strcmp(attr->name, "value_strings") == 0) &&
            (attr->n_strings > 0)) {
          if ((y->ndim != 1) || (y->dims[0] != attr->n_strings) ||
              (y->type != ONNX_TENSOR_TYPE_STRING))
            onnx_tensor_reinit(y, ONNX_TENSOR_TYPE_STRING,
                               (int[]){attr->n_ints}, 1);
...
static int Constant_exit(onnx_node_t *n) { return 1; }

static int Constant_reshape(onnx_node_t *n) { return 1; }
