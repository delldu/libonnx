...
static void ReduceLogSumExp_int8(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
  ...
  do {
    ...
    do {
      sum +=
          expf(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
...
static void ReduceLogSumExp_int32(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_int64(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_uint8(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_uint32(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_uint64(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_float32(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
...
static void ReduceLogSumExp_float64(onnx_node_t *n) {
  operator_pdata_t *pdat = (operator_pdata_t *)n->priv;
  onnx_tensor_t *x = n->inputs[0];
  onnx_tensor_t *y = n->outputs[0];
  ...
  do {
    ...
    do {
      sum +=
          exp(px[o + dim_offset(pdat->naxes, iter_in_axes, in_axes_axis_dis)]);
    } while (dim_next(pdat->naxes, iter_in_axes, iter_in_axes_max));
    py[i++] = log(sum);
  } while (dim_next(not_in_axes_num, iter_not_in_axes, iter_not_in_axes_max));
}
