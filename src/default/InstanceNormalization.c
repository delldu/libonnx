...
static void InstanceNormalization_float16(onnx_node_t *n) {
  ...
  for (j = 0; j < NC; j++) {
    ...
    var = temp / channel;
    for (i = o; i < l; i++)
      py[i] = float32_to_float16(float16_to_float32(pscale[jc]) *
                                     ((float16_to_float32(px[i]) - mean) /
                                      sqrtf(var + pdat->epsilon)) +
...
static void InstanceNormalization_float32(onnx_node_t *n) {
  ...
  for (j = 0; j < NC; j++) {
    ...
    for (i = o; i < l; i++)
      py[i] =
          pscale[jc] * ((px[i] - mean) / sqrtf(var + pdat->epsilon)) + pb[jc];
  }
}
...
static void InstanceNormalization_float64(onnx_node_t *n) {
  ...
  for (j = 0; j < NC; j++) {
    ...
    for (i = o; i < l; i++)
      py[i] =
          pscale[jc] * ((px[i] - mean) / sqrt(var + pdat->epsilon)) + pb[jc];
  }
}
