...
static int AveragePool_init(onnx_node_t *n) {
  ...
  if ((n->ninput == 1) && (n->noutput == 1)) {
    ...
    if (pdat) {
      ...
      pdat->count_include_pad =
          onnx_attribute_read_int(n, "count_include_pad", 0);
...
static int AveragePool_reshape(onnx_node_t *n) {
  ...
  for (i = 0; i < ndim - 2; i++) {
    ...
    case AUTO_PAD_NOTSET:
      if (pdat->ceil_mode)
        dims[i + 2] =
            ceilf((x->dims[i + 2] + pdat->cpads[i] +
                   pdat->cpads[i + pdat->nkernel] - pdat->kernels[i]) /
                      (float)pdat->strides[i] +
                  1);
      else
        dims[i + 2] =
            floorf((x->dims[i + 2] + pdat->cpads[i] +
                    pdat->cpads[i + pdat->nkernel] - pdat->kernels[i]) /
                       (float)pdat->strides[i] +
                   1);
      break;
    ...
    case AUTO_PAD_SAME_LOWER:
      ...
      break;
    case AUTO_PAD_VALID:
      dims[i + 2] = ceilf((x->dims[i + 2] - pdat->kernels[i] + 1) /
                          (float)pdat->strides[i]);
      break;
    default:
      break;
