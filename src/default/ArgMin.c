...
static int ArgMin_init(onnx_node_t *n) {
  ...
  if ((n->ninput == 1) && (n->noutput == 1)) {
    ...
    if (pdat) {
      ...
      pdat->select_last_index =
          onnx_attribute_read_int(n, "select_last_index", 0);
      n->priv = pdat;
      return 1;
    }
  }
...
static void ArgMin_int8(onnx_node_t *n) {
  ...
  while (idx < len) {
    if (cnt < pdat->stride) {
      for (minv = px[idx], mini = 0, i = 1, p = px + idx + pdat->stride;
           i < pdat->dim; i++, p += pdat->stride) {
...
static void ArgMin_bfloat16(onnx_node_t *n) {
  ...
  while (idx < len) {
    if (cnt < pdat->stride) {
      for (minv = bfloat16_to_float32(px[idx]), mini = 0, i = 1,
          p = px + idx + pdat->stride;
           i < pdat->dim; i++, p += pdat->stride) {
...
static void ArgMin_float16(onnx_node_t *n) {
  ...
  while (idx < len) {
    if (cnt < pdat->stride) {
      for (minv = float16_to_float32(px[idx]), mini = 0, i = 1,
          p = px + idx + pdat->stride;
           i < pdat->dim; i++, p += pdat->stride) {
