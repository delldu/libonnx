...
static int Conv_reshape(onnx_node_t *n) {
  ...
  case AUTO_PAD_SAME_UPPER:
    for (i = 0; i < pdat->npad / 2; i++) {
      pad = (ceilf(x->dims[i + 2] / (float)pdat->strides[i]) - 1) *
                pdat->strides[i] +
            ((pdat->kernels[i] - 1) * pdat->dilations[i] + 1) - x->dims[i + 2];
      pdat->cpads[i] = pad / 2;
  ...
  for (i = 0; i < ndim - 2; i++) {
    switch (pdat->auto_pad) {
    case AUTO_PAD_NOTSET:
      dims[i + 2] = floorf((x->dims[i + 2] + pdat->cpads[i] +
                            pdat->cpads[i + pdat->nkernel] -
                            ((pdat->kernels[i] - 1) * pdat->dilations[i] + 1)) /
                               (float)pdat->strides[i] +
                           1);
      break;
    case AUTO_PAD_SAME_UPPER:
    case AUTO_PAD_SAME_LOWER:
    ...
    case AUTO_PAD_VALID:
      dims[i + 2] =
          ceilf((x->dims[i + 2] -
                 ((pdat->kernels[i] - 1) * pdat->dilations[i] + 1) + 1) /
                (float)pdat->strides[i]);
      break;
    default:
      break;
...
static inline void dgemm_float32(int n, int m, int o, float *A, float *B,
                                 float *C) {
...
static inline void dgemm_float64(int n, int m, int o, double *A, double *B,
                                 double *C) {
          ...
          if (pxcache) {
            for (int n = 0; n < oN; ++n) {
              for (int group_c = 0; group_c < oC * pdat->group / M; ++group_c) {
                ...
                for (int i = (base_h < 0 ? (-base_h) / pdat->dilations[0] : 0);
                     i < H; ++i) {
                  int input_h = base_h + i * pdat->dilations[0];
                  if (input_h >= iH)
                    break;
                  for (int j =
                           (base_w < 0 ? (-base_w) / pdat->dilations[1] : 0);
                       j < W; ++j) {
                    ...
                    for (int w_channel = 0; w_channel < C; ++w_channel) {
                      ch = base_c + w_channel;
                      ((pxcachetype)pxcache)[n][ch][i][j] = float16_to_float32(
                          ((pxtype)px)[n][ch][input_h][input_w]);
                    }
                  }
                }
              }
          ...
          for (int n = 0; n < oN; ++n) {
            for (int c = 0; c < oC; ++c) {
              ...
              for (int i = (base_h < 0 ? (-base_h) / pdat->dilations[0] : 0);
                ...
                if (input_h >= iH)
                  break;
                for (int j = (base_w < 0 ? (-base_w) / pdat->dilations[1] : 0);
                     j < W; ++j) {
                  int input_w = base_w + j * pdat->dilations[1];
                  if (input_w >= iW)
                    break;
                  for (int w_channel = 0; w_channel < C; ++w_channel) {
                    ...
                    if (pxcache) {
                      ...
                      v = float16_to_float32(
                          ((pxtype)px)[n][ch][input_h][input_w]);
                    }
                    weight =
                        float16_to_float32(((pwtype)pw)[c][w_channel][i][j]);
                    sum += v * weight;
                  }
                }
              }
          ...
          for (size_t c = 0; c < C; c++) {
            for (size_t h = 0; h < H; h++) {
              for (size_t w = 0; w < W; w++) {
                ((mwtype)matw)[c * H * W + h * W + w][m] =
                    float16_to_float32(((pwtype)pw)[g * MM + m][c][h][w]);
              }
            }
          }
        }
                ...
                for (size_t h = 0; h < H; h++) {
                  for (size_t w = 0; w < W; w++) {
                    ...
                    if (ih < 0 || iw < 0 || ih >= iH || iw >= iW) {
                      ...
                      ((mxtype)matx)[hh * oW + ww][c * H * W + h * W + w] =
                          float16_to_float32(
                              ((pxtype)px)[n][g * CC + c][ih][iw]);
                    }
                  }
                }
              }
...
static void Conv_float64(onnx_node_t *n) {
  ...
  if (ndim == 4) {
    ...
    if (matw && matx && maty) {
      ...
      /* then try cached conv */
      pxcache =
          malloc(oN * (oC * pdat->group / M) * C * H * W * sizeof(double));
      if (pxcache) {
        conv_mode = CONV_CACHED;
      }
    }
