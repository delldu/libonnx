...
static void testcase(const char *path, onnx_resolver_t **r, int rlen) {
  onnx_context_t *ctx;
  onnx_tensor_t *t, *o;
  struct stat st;
  ...
  if (ctx) {
    ...
    while (1) {
      ...
      printf("%*s\r\n", 100 + 12 - 6 - len,
             ((ninput + noutput == okay) && (okay > 0))
                 ? "\033[42;37m[OKAY]\033[0m"
                 : "\033[41;37m[FAIL]\033[0m");
      data_set_index++;
    }
...
int main(int argc, char *argv[]) {
  ...
  hmap_sort(m);
  hmap_for_each_entry(e, m) { testcase(e->key, NULL, 0); }
  hmap_free(m, NULL);
  return 0;
}
